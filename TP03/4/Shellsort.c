...
void ordenar()
{
    ...
    {
        h /= 3;
        // Para cada H, executa inserção para cada pseudoarray
        // Sendo os pseudoarrays apenas iniciando na próxima posição do anterior
        // Até que essa posição atinja H
        for (int i = 0; i < h; i++)
        {
            insercao(i, h);
        }
...
int main()
{
    FILE *log = fopen("756293_shellsort.txt", "w"); // arquivo para log
    char entrada[100];
