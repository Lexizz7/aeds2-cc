...
class Fila {
    ...
    Fila(int maximo) {
        // Tamanho -> manter controle para remover quando atingir o maximo
        primeiro = new Celula();
        ultimo = primeiro;
        this.maximo = maximo;
        tamanho = 0;
    ...
    int inserir(String html) throws Exception {
        // Caso esteja cheio, remove o primeiro da fila e insere
        if (tamanho == maximo)
            remover();

        // Célula que será inserida
        Celula tmp = new Celula(html);

        // Faz a nova celula ser o proximo após o ultimo
        ultimo.prox = tmp;
        // Faz a nova celular ser a ultima
        ultimo = tmp;

        tamanho++;

        // Retorna duração média
        return duracaoMedia();
    }
    ...
    Filme remover() throws Exception {
        ...
        Celula tmp = primeiro;
        // Como o primeiro aponta para uma célula sentinela
        // O que será removido é o seu proximo, por isso
        // tmp.prox.elemento
        Filme res = tmp.prox.elemento;

        // Avança o primeiro, que é sentinela
        primeiro = tmp.prox;

        // Apaga o "primeiro" antigo
        tmp.prox = null;
        tmp = null;
    ...
    int duracaoMedia() {
        int res = 0;
        int count = 0;
        // Percorre toda a fila, somando a quantidade de duração e contando quantos
        // filmes
        for (Celula i = primeiro.prox; i != null; i = i.prox) {
            res += i.elemento.getDuracao();
            count++;
        }

        // Retorna a divisão entre eles, arredondando
        return (int) Math.round((double) res / count);
    }

    // Percorre todos os elementos chamando o método de imprimir
        ...
        try {
            ...
            do {
                ...
                if (!entrada.equals("FIM")) {
                    // Preencher a lista e imprimir a duracao media
                    out.println(filmes.inserir(entrada));
                }
            ...
            for (int i = sc.nextInt(); i >= 0; i--) {
                ...
                switch (entrada.split(" ")[0]) {
                    case "I":
                        ...
                        // Printa a duração retornada
                        out.println(filmes.inserir(entrada.split(" ", 2)[1]));
                        break;
                    case "R":
                        // Retorna apenas o nome daquele que foi removido
                        out.println("(R) " + filmes.remover().getNome());
                        break;
                }
            }
