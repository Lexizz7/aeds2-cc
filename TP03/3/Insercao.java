...
class Lista {
    ...
    void ordenar() throws Exception {
        File log = new File("756293_insercao.txt");
        log.createNewFile();
        int comparacoes = 0;
        int movimentacoes = 0;
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy"); // Para calcular qual data vem primeiro
        // Percorre todo o array, sendo o primeiro elemento já ordenado
        for (int i = 1; i < n; i++) {
            ...
            while ((j >= 0) && sdf.parse(array[j].getDataLancamento()).after(sdf.parse(tmp.getDataLancamento()))) {
                comparacoes++;
                movimentacoes++;
