...
class Lista {
    ...
    void insertion(Filme[] array) {
        for (int i = 1; i < n; i++) {
            Filme tmp = array[i]; // Guarda o elemento que está sendo comparado
            int j = i - 1; // J sempre o elemento que da parte ordenada
            // Continuar abaixando o J até que chegue a 0 ou até que encontre uma data menor
            while ((j >= 0) && array[j].getDuracao() > tmp.getDuracao()
                    || (array[j].getDuracao() == tmp.getDuracao() && array[j].getNome().compareTo(tmp.getNome()) > 0)) {
                array[j + 1] = array[j]; // Caso ainda não seja 0 e ainda não encontrou uma data menor
                                         // Jogar o elemento de trás pra frente
                j--;
            }
            array[j + 1] = tmp;
        }
    ...
    void ordenar() throws Exception {
        File log = new File("756293_countingsort.txt");
        log.createNewFile();
        int comparacoes = 0;
        int movimentacoes = 0;
        ...
        int maior = 0;
        for (int i = 0; i < n; i++) {
            if (array[i].getDuracao() > maior)
                maior = array[i].getDuracao();
        }
        maior++; // Para criar o número correto de posições

        int[] contagem = new int[maior];
        Filme[] ordenado = new Filme[n];
        ...
        // Preencher contagem
        for (int i = 0; i < n; contagem[array[i].getDuracao()]++, i++)
            ;
        ...
        // Ordenar o array
        for (int i = n - 1; i >= 0; ordenado[contagem[array[i].getDuracao()]
                - 1] = array[i], movimentacoes++, contagem[array[i]
                        .getDuracao()]--, i--)
            ;
        ...
        array = ordenado; // Apontar o array para o novo ordenado
