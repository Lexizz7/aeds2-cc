...
// Função para percorrer o array e trazer o menor titulo
int menorIndex(int i)
{
    // Caso chegue ao tamanho do array, retornar n - 1
    if (i >= n)
        return (n - 1);

    int menor = i;             // O menor primeiramente é o próprio i
    int j = menorIndex(i + 1); // Chama a função novamente

    comparacoes++;
    if ((strcmp(array[i].tituloOriginal, array[j].tituloOriginal) > 0) ||
        (strcmp(array[i].tituloOriginal, array[j].tituloOriginal) == 0 &&
         strcmp(array[i].nome, array[j].nome) > 0))
    {
        menor = j; // Caso J seja menor, trocar de lugar com I
    }

    return menor;
}

void ordenarPrivate(int i)
{
    // Seleção
    // Condição de parada = index menor que n - 1 (pois o último já estará ordenado)
    if (i < n - 1)
    {
        // Chama a função menorIndex para percorrer e retornar o menor
        int menor = menorIndex(i);

        if (i != menor) // caso não seja o menor, quer dizer que é maior
        {
            movimentacoes++;
            swap(i, menor); // trocar
        }

        ordenarPrivate(i + 1); // chama para o próximo elemento
    }
}

// para chamar ordernar sem passar parametro
void ordenar()
{
    ordenarPrivate(0);
}
...
int main()
{
    FILE *log = fopen("756293_selecaoRecursiva.txt", "w"); // arquivo para log
    char entrada[100];
