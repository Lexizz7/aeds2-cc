...
class Lista {
    ...
    void ordenar() throws Exception {
        File log = new File("756293_mergesort.txt");
        log.createNewFile();
    ...
    void mergesort(int esq, int dir) {
        // Condição de parada = esquerda e direita não podem ser iguais
        if (esq < dir) {
            int meio = (esq + dir) / 2;
            mergesort(esq, meio); // Quebrar pra esquerda
            mergesort(meio + 1, dir); // Quebrar pra direita
            intercalar(esq, meio, dir); // Intercalar
        }
    }

    void intercalar(int esq, int meio, int dir) {
        // Descobrir o tamanho de cada subarray
        int nSubEsq = (meio + 1) - esq;
        int nSubDir = dir - meio;

        // Criar subarrays
        Filme[] subEsq = new Filme[nSubEsq];
        Filme[] subDir = new Filme[nSubDir];

        // Inicializar
        for (int i = esq, j = 0; i <= meio; subEsq[j] = array[i], i++, j++)
            ;

        for (int i = meio + 1, j = 0; i <= dir; subDir[j] = array[i], i++, j++)
            ;

        // Intercalar
        // Percorrer da esquerda à direita, atribuindo ao array os menores valores
        // primeiro
        // Caso o tamanho de um dos subarrays acabe, continuar só atribuindo os valores
        // do outro
        for (int i = esq, iEsq = 0, iDir = 0; i <= dir; i++) {
            if (iEsq < nSubEsq && iDir < nSubDir) {
                comparacoes++;
                array[i] = (subEsq[iEsq].getOrcamento() <= subDir[iDir].getOrcamento()) ? subEsq[iEsq++]
                        : subDir[iDir++];
            } else if (iEsq < nSubEsq && !(iDir < nSubDir))
                array[i] = subEsq[iEsq++];
            else if (!(iEsq < nSubEsq) && iDir < nSubDir)
                array[i] = subDir[iDir++];
        }
