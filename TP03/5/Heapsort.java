...
class Lista {
    ...
    void ordenar() throws Exception {
        File log = new File("756293_heapsort.txt");
        log.createNewFile();
        int comparacoes = 0;
        int movimentacoes = 0;
        ...
        // Construção do heap
        // Começa a percorrer da segunda posição para ordenar de acordo com o filho
        // 1 -> pai de 2
        for (int i = 2; i <= n; i++) {
            comparacoes++;
            // Enquanto o filho for maior que o pai, trocar eles de lugar
            while (i > 1 && array[i].getGenero().compareTo(array[i / 2].getGenero()) > 0) {
                comparacoes++;
                movimentacoes++;
                swap(i, i / 2);
                i /= 2; // Buscar próximo pai
            }
        }
        // Reconstrução
        // Nessa parte é feita a ordenação do heap construído
        int tam = n;
        while (tam > 1) {
            movimentacoes++;
            // Trocar o último com o primeiro e diminuir o tamanho do heap
            swap(1, tam--);
            int i = 1;
            // Enquanto i tiver filhos
            while ((i * 2) <= tam) {
                int maiorFilho = 1;
                // Encontrar o maior filho pra substituir o pai
                if ((i * 2) + 1 <= tam) { // Se houver 2 filhos
                    comparacoes++;
                    // Comparar os dois e atribuir o maior
                    maiorFilho = array[i * 2].getGenero().compareTo(array[(i * 2) +
                            1].getGenero()) > 0 ? i * 2
                                    : (i * 2) + 1;
                } else {
                    // Caso tenha apenas um filho, ele é o maior
                    maiorFilho = i * 2;
                }
                comparacoes++;
                // Comparar o maior filho com o pai
                if (array[i].getGenero().compareTo(array[maiorFilho].getGenero()) < 0) {
                    movimentacoes++;
                    // Caso o filho seja maior, troque-os
                    swap(i, maiorFilho);
                    i = maiorFilho;
                } else {
                    // Caso não seja maior, sai do loop
                    i = tam;
                }
            }
        }
