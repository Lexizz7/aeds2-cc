...
class Lista {
    ...
    // serve pra chamar o método privado, com os parametros certos
    // e para criar arquivo de log
    public void ordenar() throws Exception {
        File log = new File("756293_quicksort.txt");
        log.createNewFile();

        long tempoInicial = System.currentTimeMillis();
        ordernar(0, n - 1); // chama a ordenação propriamente dita
    ...
    // Quicksort
    // Percorre um subarray começando de esq e acabando em dir
    // Seleciona sempre o centro para ser o pivo
    // Percorre primeiro a esquerda até encontrar um valor maior que o pivô
    // Ao encontrar, percorre a direita até encontrar um valor menor que o pivô
    // Se encontrar, troca os dois elementos e repete este processo até que a
    // esquerda seja maior que a direita
    // Depois, chama a função novamente com o subarray da esquerda e o subarray da
    // direita, caso existam.
    private void ordernar(int esq, int dir) {
        int i = esq, j = dir, pivo = (esq + dir) / 2; // i = começo, j = final, pivo = centro
        ...
        while (i <= j) { // enquanto a posição mais a esquerda não atinge a posição mais a direita
            ...
            while (array[i].getSituacao().compareTo(array[pivo].getSituacao()) < 0 || ((array[i]
                    .getSituacao().compareTo(array[pivo].getSituacao()) == 0)
                    && (array[i]
                            .getNome().compareTo(array[pivo].getNome()) < 0))) {
                comparacoes++;
                i++;
            }
            // percorre a direita, depois de encontrar um valor a esquerda, procurando
            // um valor menor que o pivo
            while (array[j].getSituacao().compareTo(array[pivo].getSituacao()) > 0 || ((array[j]
                    .getSituacao().compareTo(array[pivo].getSituacao()) == 0)
                    && (array[j]
                            .getNome().compareTo(array[pivo].getNome()) > 0))) {
                comparacoes++;
