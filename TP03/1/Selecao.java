...
class Lista {
    ...
    void ordenar() throws Exception {
        File log = new File("756293_selecao.txt");
        log.createNewFile();
        int comparacoes = 0;
        int movimentacoes = 0;

        // Ordenar por seleção
        // Percorre até n - 1 pois o último sempre estará em ordem
        long tempoInicial = System.currentTimeMillis();
        for (int i = 0; i < n - 1; i++) {
            int menor = i; // Declara o primeiro como menor
            for (int j = i + 1; j < n; j++) { // Percorre todos os outros até encontrar o menor, trocando-o
                comparacoes++;
                if ((array[j].getTituloOriginal().compareTo(array[menor].getTituloOriginal()) < 0) || ((array[j]
                        .getTituloOriginal().compareTo(array[menor].getTituloOriginal()) == 0)
                        && (array[j]
                                .getNome().compareTo(array[menor].getNome()) < 0))) { // Esse if gigante compara o
                                                                                      // titulo original e, caso empate,
                                                                                      // compara o nome
                    menor = j;
                }
            }
            movimentacoes++;
            swap(i, menor); // Troca as posições e continua o processo
        }
