...
class Hash {
    ...
    private int tamanho, reservaPos;

    Hash() {
        this(21, 9);
    }

    Hash(int tamanho, int reserva) {
        this.tamanho = tamanho;
        this.reservaPos = tamanho;

        this.tabela = new Filme[tamanho + reserva];
        // Inicializar tabela
        for (int i = 0; i < tabela.length; tabela[i] = null, i++)
            ;
    }

    // Soma todos os caracteres e atribui a uma posição dentro do intervalo
    // 0-tamanho
    public int hash(String string) {
        int res = 0;
        for (int i = 0; i < string.length(); res += string.charAt(i), i++)
            ;
        return res % this.tamanho;
    }

    // Recebe o html, cria o filme e caso a posição de hash não exista conteúdo
    // insere nela mesmo, caso exista, insere na reserva
    public void inserir(String html) throws Exception {
        ...
        if (tabela[pos] == null)
            tabela[pos] = tmp;
        else if (reservaPos < tabela.length)
            tabela[reservaPos++] = tmp;
    }

    // Vai direto na posição retornada pela função hash
    // Caso o conteudo dessa posição não seja igual a chave
    // Procura sequencialmente na área reserva
    public int indexOf(String chave) {
        int pos = hash(chave);
        int res = -1;

        if (tabela[pos] != null && chave.equals(tabela[pos].getTituloOriginal())) {
            res = pos;
        } else {
            for (int i = this.tamanho; i < tabela.length; i++) {
                if (tabela[i] != null && chave.equals(tabela[i].getTituloOriginal())) {
                    res = i;
                    i = tabela.length;
                }
            }
