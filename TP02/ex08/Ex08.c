...
// Todos os métodos seguem a mesma lógica do Exercício 5 em java, comentei apenas as diferenças
...
int main()
{
    ...
    for (int i = qntComando; i >= 0; i--)
    {
        ...
        char *tipo;                      // o tipo (II, IF, RI, RF, I*, R*)
        ...
        if (strstr(tipo, "II") != NULL)
        {
            inserirInicio(trim(replaceAll(entrada, "II", "")));
        }
        else if (strstr(tipo, "IF") != NULL)
        {
            inserirFim(trim(replaceAll(entrada, "IF", "")));
        }
        else if (strstr(tipo, "I*") != NULL)
        {
            // pega a posição que deseja, alocando 15000 pois é o necessário para o parametro da função replaceAll
            char *pos;
            pos = malloc(sizeof(char) * 15000);
            strcpy(entrada, trim(replaceAll(entrada, "I*", "")));
            strncpy(pos, entrada, 2);

            inserir(strcat(trim(replaceAll(entrada, pos, "")), "l"), atoi(pos));
        }
        else if (strstr(tipo, "RI") != NULL)
        {
            printf("(R) %s\n", removerInicio().nome);
        }
        else if (strstr(tipo, "RF") != NULL)
        {
            printf("(R) %s\n", removerFim().nome);
        }
        else if (strstr(tipo, "R*") != NULL)
        {
            // Aqui não é necessario alocar 15000 pois não chama a função replaceAll
            char *pos;
            pos = malloc(sizeof(char) * 2);
            strcpy(entrada, trim(replaceAll(entrada, "R*", "")));
            strncpy(pos, entrada, 2);

            printf("(R) %s\n", remover(atoi(pos)).nome);
        }
    }
