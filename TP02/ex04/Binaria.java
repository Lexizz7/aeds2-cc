...
class Lista {
    private String[] arr;
    private int n;
    private int comparacoesPesquisa = 0;// comparações feitas durante a pesquisa binaria
    ...
    // método de pesquisa binaria
    // levei em consideração que o pub.in já estava ordenado e não ordenei
    boolean existe(String pesquisa) {
        ...
        // compara a string de pesquisa com o atributo do meio da lista
        // se o elemento central for maior (ordem alfabetica) que o elemento de pesquisa
        // significa que a resposta está antes (para a esquerda do array)
        // o contrário para caso o elemento central seja menor
        while (esq <= dir) {
            meio = (esq + dir) / 2; // calcula a posicao central
            if (arr[meio].compareTo(pesquisa) < 0) { // se for maior que o elemento central
                comparacoesPesquisa += 1;
                esq = meio + 1; // limitar o array criando um subarray só com a parte maior
            } else if (arr[meio].compareTo(pesquisa) > 0) { // se for menor que o elemento central
                comparacoesPesquisa += 2;
                dir = meio - 1; // traz a extremidade direita para o centro, criando um subarray só com os
                                // valores menores
            } else { // caso seja igual, é pq encontrou
                comparacoesPesquisa += 2;
                res = true;
                esq = dir + 1; // sair do loop
            }
        }
...
class Binaria {
    ...
    public static void main(String[] args) {
        ...
        try { // criação do log
            File log = new File(matricula + "_binaria.txt");
